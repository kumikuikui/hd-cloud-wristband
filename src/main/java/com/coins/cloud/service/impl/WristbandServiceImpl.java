...
public class WristbandServiceImpl implements WristbandService {
    ...
    public List<UserDeviceBo> getRecordByCode(int userId, String mac, String code,
        ...
        for (UserDeviceBo userDeviceBo : list) {
            ...
            if (temp.equals(DeviceConfig.con003)) {//BMI
                log.info("aa-value: {},  ", userDeviceBo.getValue());

                String bp_val = userDeviceBo.getValue();

                if (!bp_val.isEmpty() && bp_val.contains("|")) {
                    String sys_dia[] = bp_val.split("\\|");
                    String bp_val_reversed = sys_dia[1].concat("|").concat(sys_dia[0]);
                    log.info("aa-new-Value: {},  ", bp_val_reversed);
                    userDeviceBo.setValue(bp_val_reversed);
                }
            }
